...
metadata<-metadata[metadata$sample_time=='Week 1',]
metadata<-metadata[metadata$sample_time=='Week 24',]
str(metadata)
#colnames(metadata)[3] = "subject_number"
levels(metadata$delivery_mode)

#metadata <- mutate(metadata, delivery_mode =recode(.x=delivery_mode, "SVD "="SVD"))
#metadata <- mutate(metadata, delivery_mode =recode(.x=delivery_mode, "CS "="CS"))
#metadata <- mutate(metadata, delivery_mode =recode(.x=delivery_mode, ""="NA"))
...
for(ml in my_level){
  ...
  ggsave(paste0("output/", ml, "BarPlot_week24", my_column, ".png"), height = 5)
}
...
alpha = 0.05
my_contrast = c("group", "SVD-FT", "SVD-PT") 
my_contrast = c("group", "SVD-FT", "CS-PT")
my_contrast = c("group", "SVD-FT", "CS-FT")
...
ggsave(paste0("output/DESeq2-week 24", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_fig, height = 5, width = 10)
